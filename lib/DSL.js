...
(function (collections) {
    ...
    // this method make the object name schemasObjec which include all the canonical uri map with the specific object explain here ----> "https://docs.google.com/document/d/12M4C2p0TpUW4GkP8zwnn1iYLxgYFYRzehYpT49BHjq4/edit#heading=h.yqpvacifw5cm"
    ...
    //this will reaturn the walker type of the key
    ...
    // perform all the operations store in the oeprationsArr function 
    ...
    //used by valueIterator method to update there storage
        ...
        for (const key in StorageObj) {
            if (StorageObj[key].hasOwnProperty("updater")) {
                ...
                for (let elm of updaters) {
                    if (this.checkConditions(elm.conditions, target, parent, StorageObj) && elm.hasOwnProperty("getters")) {
    ...
    // all the methods used to perform the operations 
    Transform.OperationsObj = {
        ...
        "addSiblingProperty" : function (main, target, parent) {
            const keyPara = main.keyPara
        ...
        'valueIterator': function (main, target, parent) {
            ...
            for (const [key, value] of Object.entries(targetArray)) {
                ...
                this.storageUpdater(key, targetArray, storageObj)
        ...
        "isType": function (mainPara, target, parent) {
            ...
            const __target = (() => {
                if (keyPara.hasOwnProperty("target")) {
                    const resolved_Target = this.gettersResolver(keyPara.target, target, parent, mainPara["storageObj"])
            ...
            const typeMethods = {
                array: () => Array.isArray(from[__target]),
                object: () => typeof from[__target] === 'object' && !Array.isArray(from[__target]),
                string: () => typeof from[__target] === 'string'
            };
