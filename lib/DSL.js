...
(function (collections) {
    ...
    Transform.prototype.applyTransformations = function () {
        for (elm of this.transformRules) {
            ...
            const jsonObjPath = elm.path ? "json/"+elm.path : "json/"+"*" 
            const pathArray = elm.path ? getArrayOfPath(jsonObjPath) : ['*']
            this.recursiveTraversal(pathArray, 0 , this.jsonObj, elm)
        }
    ...
    Transform.CondtionsObj = {
        ...
        "hasType" : function (mainPara , target , parent){
            const keyPara = mainPara.keyPara 
            const __target = (()=>{
                if (keyPara.hasOwnProperty("target") ) { 
                    const resolved_Target = this.getterResolver(keyPara.from , target , parent , mainPara["storageObj"])
