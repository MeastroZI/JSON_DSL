...
(function (collections) {
    ...
    exports.Convert = function (transformRules, jsonDocs , walkers) {
        const jsonClone = getClone(jsonDocs)
        const TransformInstance = new Transform(transformRules, jsonClone, walkers)
    ...
    function Transform(transformRules, jsonDocs , walkers) {
    ...
    Transform.prototype.applyTransformations = function () {
        for (rule of this.transformRules) {
            // the reason to do this is to make the editchildkey method to work on the root 
            // const jsonObjPath = rule.path ? "json/"+rule.path : "json/"+"*" 
            // const pathArray = rule.path ? getArrayOfPath(jsonObjPath) : ['*']

            this.recursiveTraversal(this.jsonObj, rule ) 
        }
    ...
    //prevRef  ----- >  parent
    Transform.prototype.recursiveTraversal = function ( schema, rules) {
        ...
        for (const key in schema) { 
            ...
            if (type == "array") { 
                for ( const _schema_ of schema[key]) { 
                    this.recursiveTraversal(_schema_ , rules)
                }
            }
            else if (type == "object" ){
                for (const schemaKey in schema[key]) { 
                    this.recursiveTraversal(schema[key][schemaKey] , rules)
                }
            }
            else if (type == "value") { 
                this.recursiveTraversal(schema[key] , rules)
            }
    ...
    Transform.prototype.getWalkerType = function (key , prevRef) { 
        ...
        if (typeof walkerDetail.type == 'array'){
    ...
    Transform.prototype.Controller = function (target, parent, rule) {
        ...
        for (const elm of condiOperArr) {
            if (this.checkConditions(elm.conditions, target, parent)) {
                this.storeOperations(elm.operations, target, parent)
            }
        }
    ...
    Transform.prototype.storeOperations = function (oprs, target , parent , storageObj ) { 
        for (let key in oprs) {
            ...
            const parameters = [
                 { keyPara: oprs[key], storageObj: storageObj },
                 target ,
                 parent
            ]
            this.operationsArr.push ({functionrefer:functionrefer , parameters :parameters})
        }
    ...
    Transform.OperationsObj = {
        ...
        'valueIterator': function (main, target, parent) {
            ...
            if (condtionCheckerRef.call(this, conditions, target, parent, storageObj)) {
                storeOperationsMethod.call(this, operations, target, parent, storageObj)
            }
    ...
    Transform.CondtionsObj = {
        ...
        "not" : function (mainPara , target , parent) {
            ...
            const conditionArray = keyPara.condiArr

            const from = (()=>{
                if (keyPara.hasOwnProperty("from")) {
                    const resolved_From = this.getterResolver(keyPara.from , target , parent , mainPara["storageObj"])
            ...
            const _target = (()=>{
                if (keyPara.hasOwnProperty("target")){
                    const resolved_Target = this.getterResolver(keyPara.from , target , parent , mainPara["storageObj"])
                    return resolved_Target
                }
                return target
            })

            const result = this.checkConditions(conditionArray , _target , from , mainPara["storageObj"])
