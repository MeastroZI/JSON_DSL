...
(function (collections) {
    ...
    Transform.CondtionsObj = {
        ...
        "isSchema": function (mainPara, target, parent) {
            const keyPara = mainPara.keyPara
            const uri = keyPara["path"] ? this.gettersResolver(keyPara["path"], target, parent, mainPara["storageObj"]) : target[parent]
            const fragement = Transform.Getters.getFragmentUri.call(this, { keyPara: { uri: uri } }, target, parent)
            const pathArr = getArrayOfPath(fragement)
            const rootUri = Transform.Getters.getUriWithoutFragment.call(this, { keyPara: { uri: uri } }, target, parent)
            const rootReference = Transform.Getters.getReference.call(this, { keyPara: { path: rootUri }, storageObj: mainPara["storageObj"] }, target, parent)


            let tempRef = rootReference
            let schemaFlage = false
            let schemaLevel = 0
            let index = 0

            while (index < pathArr.length) {
                const walkerType = this.getWalkerType(pathArr[index], tempRef)
                if (walkerType == undefined && schemaLevel > 0) {

                    schemaLevel = schemaLevel - 1

                    tempRef = tempRef[pathArr[index]]
                    index = index + 1


                }
                else if (schemaLevel == 0 && walkerType == "value") {
                    schemaLevel = 0
                    tempRef = tempRef[pathArr[index]]
                    index = index + 1
                }
                else if (schemaLevel == 0 && (walkerType == "array" || walkerType == "object")) {
                    tempRef = tempRef[pathArr[index]]
                    index = index + 1
                    schemaLevel = schemaLevel + 1
                }
